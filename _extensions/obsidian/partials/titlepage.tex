...
% Full-width contrast rule, used by the banded themes between the
% title and the author block. Colour, width and spacing are theme
% keys; omitted when the theme has no band-rule keys.
\newcommand{\bandrule}{%
\vspace{$if(titlepage-theme.band-rule-space)$$titlepage-theme.band-rule-space$$else$0pt$endif$}
{\color{$if(titlepage-theme.band-rule-color)$$titlepage-theme.band-rule-color$$else$gold$endif$}%
\rule{\textwidth}{$if(titlepage-theme.band-rule-width)$$titlepage-theme.band-rule-width$$else$3pt$endif$}}
\vspace{$if(titlepage-theme.band-rule-space)$$titlepage-theme.band-rule-space$$else$0pt$endif$}}
...
\pagecolor{pgcolor}\afterpage{\nopagecolor}
$endif$$endif$
$if(titlepage-theme.classification-color)$
% Banded themes sit on a dark band; the classification marking
% recolors to stay legible (defined in before-body.tex).
\renewcommand{\obsidianclasscolour}{$titlepage-theme.classification-color$}
$endif$
...
% House identity line: keeps the themed page traceable. On a dark
% band (banded themes) the line recolors to white; otherwise it uses
% the house secondary grey.
\vfill
{\centering\small\textcolor{$if(titlepage-theme.classification-color)$$titlepage-theme.classification-color$$else$obsidianl$endif$}{\docref{}
