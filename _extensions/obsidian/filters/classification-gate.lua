...
function Pandoc(doc)
  local explicit_gscp = false
  local flag = doc.meta['gscp']
  if flag then
    if type(flag) == 'boolean' then
      explicit_gscp = flag
    elseif flag.t == 'MetaBool' then
      explicit_gscp = flag.c
    else
      local s = stringify(flag):lower()
      explicit_gscp = (s == 'true' or s == 'yes' or s == 'on')
    end
  end

  -- Auto-enable GSCP when a classification is set.
  local marking = stringify(doc.meta['confidentiality'] or doc.meta['classification'] or '')
  local auto_gscp = (not explicit_gscp) and (marking ~= '')

  if not explicit_gscp and not auto_gscp then
    return doc
  end
  ...
  if OBSOLETE[base] then
    if explicit_gscp then
      ...
      io.stderr:write('classification-gate: WARNING "' .. marking
        .. '" is an obsolete classification (RESTRICTED, CONFIDENTIAL). '
        .. 'Set gscp: true to enforce GSCP validation.\n')
      doc.meta['confidentiality'] = pandoc.MetaString(marking)
      return doc
    end
  end
  ...
  if not VALID[base] then
    if explicit_gscp then
      ...
      -- Auto-enabled: non-GSCP labels pass through with a warning.
      io.stderr:write('classification-gate: "' .. marking
        .. '" is not a GSCP level. Set gscp: true to enforce '
        .. 'GSCP validation.\n')
      doc.meta['confidentiality'] = pandoc.MetaString(marking)
      return doc
    end
